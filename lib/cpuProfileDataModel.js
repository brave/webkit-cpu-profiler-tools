const {ProfileNode, ProfileTreeModel} = require('./profileTreeModel')
...
const CPUProfileDataModel = class extends ProfileTreeModel {
  ...
  _translateProfileTree(nodes) {
    ...
    var keepNatives = true
    var root = nodes[0];
    /** @type {!Map<number, number>} */
    var idMap = new Map([[root.id, root.id]]);
    var resultRoot = new CPUProfileNode(root, sampleTime);
    ...
    while (sourceNodeStack.length) {
      ...
      if (!sourceNode.children)
        sourceNode.children = [];
      var targetNode = new CPUProfileNode(sourceNode, sampleTime);
  ...
  _normalizeTimestamps() {
    ...
    for (var i = 0; i < timestamps.length; ++i)
      timestamps[i] /= 1000;
    var averageSample = (timestamps[timestamps.length - 1] - timestamps[0]) / (timestamps[timestamps.length - 1] - 1);
